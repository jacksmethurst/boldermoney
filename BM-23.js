var $ = window.jQuery;
	...
	calc_btn.click(function () {
		...
		values.map((item) => {
			...
			array_target.append(
				`<input style="color: #000;" type="text" id="quiz-${item.name}" name="quiz-${item.name}" placeholder="${item.name}" value="${item.value}"></input>`
			);
		});
		...
		function returnInput(name, value) {
			return `<input style="color: #000;" type="text" id="quiz-${name}" name="quiz-${name}" placeholder="${name}" value="${value}"></input>`;
		}
		...
		if ($("#quiz-ANNUIN").val() === "<$50k") {
			array_target.append(
				returnInput(
					"RETIREINCREASE",
					parseFloat(0.108 * 25000).toFixed(2)
				)
			);
		} else if ($("#quiz-ANNUIN").val() === "$51-100k") {
			array_target.append(
				returnInput(
					"RETIREINCREASE",
					parseFloat(0.162 * 51000).toFixed(2)
				)
			);
		} else if ($("#quiz-ANNUIN").val() === "$101-150k") {
			array_target.append(
				returnInput("RETIREINCREASE", parseFloat(19500).toFixed(2))
			);
		} else if ($("#quiz-ANNUIN").val() === "$151k-200k") {
			array_target.append(
				returnInput("RETIREINCREASE", parseFloat(19500).toFixed(2))
		...
		fetch(fetch_link, requestOptions)
			...
			.then(() =>
				window.location.replace(
					`/your-potential?aincome=${aincome_submit}&debt=${debt_submit}`
				)
			)
